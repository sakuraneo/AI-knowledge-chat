...
以 M4 `add-rag-pdf-chat` 为例，从 propose 到 archive 的 **spec 与代码** 两条线：

```mermaid
flowchart TB
  subgraph propose["① propose"]
    P[proposal.md + design.md + tasks.md]
    D[changes/.../specs/<br/>delta：ADDED / MODIFIED 等]
  end

  subgraph apply["③ apply"]
    C[server/ + client/<br/>真实代码]
  end

  subgraph archive["⑤ archive"]
    S[openspec/specs/<br/>合并后的完整 spec<br/>= 当前系统应该怎样]
    A[changes/archive/&lt;date&gt;-&lt;name&gt;/<br/>proposal + design + tasks + 原 delta<br/>= 这次变更的档案袋]
  end
...
```text
                    apply 阶段
                         │
  changes/.../specs/          server/ + client/
  （delta：只写新增/修改）        （真实实现）
                         │
                         ▼  archive
         ┌───────────────┴───────────────┐
         ▼                               ▼
  openspec/specs/              changes/archive/<date>-<name>/
  （合并后的完整 spec）          （proposal + design + tasks + 原 delta）
  = 当前系统应该怎样              = 这次变更的档案袋
```
...
| 箭头 / 节点 | 含义 |
|-------------|------|
| delta → 代码 | apply 时 AI/人按 delta + tasks 写实现 |
| delta → `openspec/specs/` | archive 时 ADDED/MODIFIED/REMOVED 写入 baseline |
| delta → `changes/archive/` | 原 change 整包保留，**不删除、不清空** |
| 代码真相 | 始终在 Git；spec 真相在 archive 后的 `openspec/specs/` |
...
| 2026-05-24 | §5.0 增加 Delta 合并总览图（mermaid + ASCII） |
